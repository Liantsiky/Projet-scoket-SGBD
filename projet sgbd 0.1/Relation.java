...
public class Relation{
    ...
    // entree des attributs
        ...
        for(int k=0;k<atransformer.getAttributs().size();k++){
            if(k==atransformer.getAttributs().size()-1){
                table=atransformer.getAttribut(k)+"/";
            }else{
                table=atransformer.getAttribut(k)+",";
            }
        }
    ...
    //Produit cartesien
        ...
        // set
            resultat.setAttributs(newAttributs);
            resultat.setLignes(lignesFinal);
        return resultat;
        }
    //Difference
    ...
    // prend l'indice de la colonne reference
        for(int i=0;i<r1.getAttributs().size();i++){
            if(r1.getAttribut(i).compareToIgnoreCase(cond[0])!=0){
                indice=i;
            }
        }
    // prend les attributs 
    ...
    // comparaison
        for(int j=0;j<r1.getLignes().size();j++){
            if(r1.getCase(j, indice).compareToIgnoreCase(cond[1])!=0){
